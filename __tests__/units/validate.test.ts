...
describe("validate.credit schema", () => {
  const validCredit = normalizeForZod(seedData.credits[0]);

  test("parses valid credit and transforms attended", () => {
    const result = validate.credit.parse(validCredit);
  ...
  test("attended defaults to false if not provided", () => {
    const credit = { ...validCredit, attended: undefined };
    const result = validate.credit.parse(credit);
    expect(result.attended).toBe(false);
  });
});

describe("validate.course schema", () => {
  const validCourse = normalizeForZod(seedData.courses[0]);
