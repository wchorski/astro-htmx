...
export const WRITABLE_FIELDS = {
  ...
  creditFull: ["user_id", "course_id", "grade", "attended"],
} as const;
...
export const userPolicy = {
  read: async (session: Session, row: BaseRow) =>
    ...
    (await userCan(session.user_id, PERMISSIONS.manageAllMembers)),
...
export const creditPolicy = {
  read: async (session: Session, row: BaseRow) =>
    session.user_id === row.id ||
    (await userCan(session.user_id, PERMISSIONS.manageAllCredits)),
  uptimestamp: async (session: Session, row: BaseRow) =>
    session.user_id === row.id ||
    (await userCan(session.user_id, PERMISSIONS.manageAllCredits)),
  writableFields: async (session: Session, row: BaseRow) => {
