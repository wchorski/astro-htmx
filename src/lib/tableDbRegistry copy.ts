...
const memberCreateFormSchema = z.object({
  first_name: z.string().min(3, "Must be more than 3 characters"),
  last_name: z.string().min(3, "Must be more than 3 characters"),
  middle_initial: z.string().max(1, "no more than one character").optional(),
  phone: z.string().min(10, "Phone must be at least 10 digits"),
...
export const crudRegistry = {
  ...
  // },
  // TODO convert to a nested objects like members.update and members.create
  members: {
    // create new member
    create: async (row) => {
      row.phone = formatPhoneToE164Manual(row.phone);
      if (!row.phone) throw new Error("phone number improper format");
      const validated = memberCreateFormSchema.parse(row);

      const existingMember = validated.phone
        ? await db
            .select()
            .from(Member)
      ...
      if (existingMember)
        throw new ConflictError(
          `member with phone and or email already exists in database`,
        );

      const [newMember] = await db.insert(Member).values(validated).returning();
      console.log({ newMember });
      return newMember;
    },
    read: async (id) => {
      return await db.select().from(Member).where(eq(Member.id, id)).get();
    },
    update: async (row) => {
      const { id, ...fields } = row;
      const [result] = await db.update(Member).set(fields).where(eq(Member.id, id)).returning();
      return result;
    },
    delete: async (id) => {
...
export type TableRegistryType = keyof typeof crudRegistry;
